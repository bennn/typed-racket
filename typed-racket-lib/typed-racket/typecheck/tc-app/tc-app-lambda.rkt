...
#|bg|#   (let ([r (tc/let-values #'((x) ...) #'(args ...) #'body expected)])
#|bg|#     (define arg-ts
#|bg|#       (for/list ([arg (in-list (syntax-e #'(args ...)))])
#|bg|#         (match (type-of arg)
#|bg|#          [(tc-result1: t) t])))
#|bg|#     (define return-type (bg-parse-return r))
#|bg|#     (add-typeof-expr #'lam (ret (->* arg-ts return-type)))
#|bg|#     r)
